...
#%%
df = pd.read_excel("warangal_rural.xlsx", sheet_name = 'kharif')
...
for crop_num in range(len(crop_list)):
    df_all['normalAreaSown'][mandal_num*crop_num:mandal_num+mandal_num*crop_num] = df[col_list[2+crop_num*6]][3:]
    df_all['actualAreaSown'][mandal_num*crop_num:mandal_num+mandal_num*crop_num] = df[col_list[3+crop_num*6]][3:]
...
#%%
df_all.to_csv('warangalrural_Kharif_2016-17.csv', index=False)
