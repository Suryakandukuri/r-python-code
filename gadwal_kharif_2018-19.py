...
row_skip = [16, 17, 18, 19]
df = pd.read_excel('Kharif 2018-19 Normal Areas.xlsx', skiprows=row_skip)
...
# Importing numpy to get nan values
import numpy as np
# Normal and actual sown
for crop_num in range(len(crop_list)):
    df_all['Normal'][mandal_num*crop_num:mandal_num+mandal_num*crop_num] = df[col_list[2+crop_num]][3:]
    df_all['Actual'][mandal_num*crop_num:mandal_num+mandal_num*crop_num] = np.nan
...
# Export to csv file
df_all.to_csv('gadwal_kharif_2018-19.csv', index=False)
